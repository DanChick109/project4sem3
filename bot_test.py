...
def list_of_t(message) -> None:
    ...
    kafs = []
    print(mmsg_txt, '\n', f"SELECT kaf_name FROM kafedra WHERE split_part(kaf_name, ' ', ) = '({mmsg_txt})'")
    if len(mmsg_txt) < 6:
        ...
        cursor.execute(f"SELECT kaf_name FROM kafedra WHERE kaf_name = '{mmsg_txt.capitalize()}'")
    ...
    try:
        if len(message.text.split()) == 2:   ### Готов
            for i in range(1, 3+1):
                ...
                if teacher: break
        elif len(message.text.split()) == 3:   ### Готов
            ...
            teacher = list(cursor.fetchall())
        if len(message.text.split()) == 1: bot.send_message(message.chat.id, text="Имя препода необходимо ввести после '/teacher'")
        if not teacher and len(message.text.split())>1: bot.send_message(message.chat.id, text="Такого препода нет")
...
def handle_butt(message) -> None:   ### Готов
    ...
    try:
        ...
        elif message.text == 'Убрать клавиатуру': bot.send_message(message.chat.id, 
